...
- **Absolute Sentinel Authority**: The Sentinel now acts as the absolute physical authority. Even if a hardcoded `batch_size` exists in the YAML config, the suite treats it as a maximum boundary and will autonomously throttle it downwards if the local physical VRAM cannot hold it, permanently preventing system RAM paging.
...
- **Sub-Nuclear 4GB Lockdown (v22.0)**: On GTX 1650 cards, the suite enforces a strict **Serial-Only Mode** (0 workers) after an OOM. The absolute Memory Sentinel clamps pixel volumes to fit entirely within physical VRAM, delivering a **2x performance gain** by preventing Windows System RAM paging.
...
- **Universal Quality Scorecarding**: Lifted structural-perceptual score evaluations out of quality-only constraints. The trainer now dynamically compounds PSNR, SSIM, LPIPS, and FID for all restoration models. **Official academic SOTA benchmarks** (FFANet, MIRNet, MPRNet) have been natively injected into the registry, allowing the governor to mathematically verify ultimate convergence.
- **Global Fraction Baseline (15%)**: All fresh models now bootstrap at a hyper-light `15%` dataset fraction baseline (down from 50%), allowing the foundation phase to establish initial manifold geometry at blazing speed without manual fraction overrides.
