...
- **Professional Multi-Task Restoration (v23.3)**: Fully integrated multi-headed Mixture-of-Experts (MoE) routing engine with 6 specialized restoration heads (denoise, deblur, derain, dehaze, lowlight, superres) dynamically trained under **Dynamic Filename Task Ingestion** to prevent task-routing collapse. Deployed **Downsampled Global Attention (DGA)** to execute global spatial self-attention under a dense $32 \times 32$ pooled resolution, achieving complete OOM immunity and 4000x spatial reduction on GTX 1650/4GB hardware.
