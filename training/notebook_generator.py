...
def generate_inference_notebook(model_key, export_dir, unified_models_registry=None, config=None):
    ...
    stealth_source = [
        "import os, base64, torch, subprocess, glob, shutil\n",
        f"model_key = '{model_key}'\n",
        "hub_root = '/kaggle/working/LemGendaryModels'\n",
        ...
        "try:\n",
        "    res = subprocess.run(f\"find /kaggle/input -maxdepth 4 -type f -path '*/checkpoints/*' -name '*{model_key.replace('_', '*')}*.pth'\", shell=True, capture_output=True, text=True).stdout.strip().split('\\n')\n",
        "    input_ckpts = [p for p in res if p]\n",
        "except: input_ckpts = []\n",
        "\n",
        "if input_ckpts:\n",
        ...
        "\n",
        "    # 2026 Resilience: Recover Metrics Audit Trail\n",
        "    try:\n",
        "        met_res = subprocess.run(f\"find /kaggle/input -maxdepth 4 -type f -name 'metrics.csv' -path '*/{model_key.replace('_', '*')}*'\", shell=True, capture_output=True, text=True).stdout.strip().split('\\n')\n",
        "        src_met = next((p for p in met_res if p and os.path.exists(p)), None)\n",
        "        if src_met:\n",
    ...
    checkpoint_recovery_source = [
        ...
        "\n",
        "try:\n",
        "    from training.train import unified_models_registry # Optional: fallback to local keys if registry fails\n",
        "    reg_filename = unified_models_registry.get(model_key, {}).get('filename', '')\n",
        "except: reg_filename = ''\n",
        "\n",
        ...
        "    \n",
        "    metrics_found = False\n",
        "    # Find metrics.csv near the checkpoints\n",
        "    for src in found_ckpts:\n",
