...
def generate_inference_notebook(model_key, export_dir, unified_models_registry=None, config=None):
    ...
    hub_prep_source = [
        ...
        "    # 2026: Use blob filtering to avoid pulling ALL large files in the history\n",
        "    res = subprocess.run(['git', 'clone', '--depth', '1', '--filter=blob:none', hub_url, hub_root], capture_output=True, text=True)\n",
        ...
        "    subprocess.run(['git', 'pull', 'origin', 'main'], cwd=hub_root)\n",
