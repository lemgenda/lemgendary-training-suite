...
def generate_inference_notebook(model_key, export_dir, unified_models_registry=None, config=None):
    ...
    if kaggle_accel == "P100":
        accel_str = "GPU P100 (Single NVIDIA Tesla P100 - 16GB HBM2 732 GB/s)"
    elif kaggle_accel in ["Dual T4 or P100", "P100 or Dual T4"]:
        accel_str = "GPU P100 (Single-Die HBM2) or GPU T4 x2"
    elif kaggle_accel == "Dual T4":
        accel_str = "GPU T4 x2 (Dual NVIDIA Tesla T4 - 30GB total VRAM)"
    else:
        accel_str = str(kaggle_accel)
...
def generate_colab_inference_notebook(model_key, export_dir, unified_models_registry=None, config=None):
    ...
    symlink_source = [
        ...
        "                    if is_match:\n",
        "                        def is_valid_ds(p):\n",
        "                            try: return os.path.exists(os.path.join(p, 'images')) or os.path.exists(os.path.join(p, 'targets')) or any(f.endswith('.csv') or f.endswith('.json') or f.endswith('.parquet') for f in os.listdir(p))\n",
        "                            except: return False\n",
        "                        if is_valid_ds(path): found.append(path)\n",
        "                        else:\n",
        "                            try:\n",
