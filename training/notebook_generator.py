...
def generate_inference_notebook(model_key, export_dir, unified_models_registry=None, config=None):
    ...
    stealth_source = [
        ...
        "try:\n",
        "    res = subprocess.run(f\"find /kaggle/input -maxdepth 8 -type f -path '*/checkpoints/*' -name '*{model_key.replace('_', '*')}*.pth'\", shell=True, capture_output=True, text=True).stdout.strip().split('\\n')\n",
        ...
        "    try:\n",
        "        met_res = subprocess.run(f\"find /kaggle/input -maxdepth 8 -type f -name 'metrics.csv' -path '*/{model_key.replace('_', '*')}*'\", shell=True, capture_output=True, text=True).stdout.strip().split('\\n')\n",
