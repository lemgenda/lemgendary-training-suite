...
def trigger_sota_export(model, args, config, unified_models_registry, epoch, plcc, srcc, psnr, ssim_val, lpips_val, fid):
    ...
    try:
        ...
        # 2026 Resilience: Auto-Mirror production binaries to the production Hub
        # We look for the binaries generated by the export scripts in the project root
        print(f"   -> Mirroring production binaries to Hub...")
        for f_name in os.listdir(os.getcwd()):
            if f_name.endswith(('.pt', '.onnx', '.json')) and args.model in f_name:
                shutil.copy2(os.path.join(os.getcwd(), f_name), os.path.join(trained_models_hub_dir, f_name))
            elif f_name.endswith('.data') and args.model in f_name:
                shutil.copy2(os.path.join(os.getcwd(), f_name), os.path.join(trained_models_hub_dir, f_name))
