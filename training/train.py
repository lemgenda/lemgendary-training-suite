            ...
            "cells": [
                ...
                },
                {
                    ...
                    "source": [
                        ...
                        "    paths.sort(key=lambda x: ('latest' in x or 'best' in x), reverse=True)\n",
                        "    \n",
                        "    # Strict Exclusion: Prevent Denoising/Deblurring cross-loading mismatch\n",
                        "    is_denoising_task = 'denoising' in args.model.lower()\n",
                        "    is_debluring_task = 'debluring' in args.model.lower() or 'deblurring' in args.model.lower()\n",
                        "    \n",
                        "    filtered_paths = []\n",
                        "    for p in paths:\n",
                        "        p_low = p.lower()\n",
                        "        if 'denoising' in p_low and not is_denoising_task: continue\n",
                        "        if ('debluring' in p_low or 'deblurring' in p_low) and not is_debluring_task: continue\n",
                        "        filtered_paths.append(p)\n",
                        "    \n",
                        "    onnx_path = next((p for p in filtered_paths if os.path.exists(p)), None)\n",
                        "    \n",
                        "    if onnx_path:\n",
